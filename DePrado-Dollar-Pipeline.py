...
Meta-Labeling, Purged CV, SADF, Entropy, Bet Sizing, Backtest net de coûts.

Usage:
    python pipe.py                          # GLD depuis 2020, signal + backtest + dashboard
...
def momentum_signal(close_vals, idx, lookback=20):
    ...
    return {"side": side, "mom_side": side, "mom_ret": ret}
...
def triple_barrier(close, events, horizon, upper_w, lower_w,
                   vol_lookback=20, mom_lookback=20):
    ...
    for t0 in events:
        ...
        sig = momentum_signal(vals, i0, mom_lookback)
        if sig is None:
            continue
...
def sadf(log_prices, min_window=10):
    ...
    for t in range(min_window, n):
        best = -np.inf
        for t0 in range(max(0, t - min_window * 10), t - min_window + 1):
...
class Pipeline:
    def __init__(self, tf="d1", multiplier=None, frac_d=0.4, horizon=None,
                 ...
                 spread_pct=None, swap_daily_pct=None, raw_bars=False, **kw):
        ...
        self.frac_d = frac_d
        self.horizon = horizon or preset["horizon"]
        self.upper_w = upper_width
        self.lower_w = lower_width
        ...
        log.info(f"Config : {preset['label']} | H={self.horizon} Mom={self.mom_lb}")
    ...
    def _build(self, path):
        ...
        log.info(f"Triple Barrière (H={self.horizon} Mom={self.mom_lb})...")
        edf = triple_barrier(close, evts, self.horizon, self.upper_w, self.lower_w,
                             VOL_WARMUP, self.mom_lb)
        if edf.empty:
    ...
    def _walk_forward(self, features, feat_cols, silent=False):
    ...
    def run_signal(self, filepath):
        ...
        signals = self._walk_forward(feat, fc)
        if not signals:
    ...
    def run_eval(self, filepath):
        ...
        try:
            ...
            if not fc or len(feat) < self.min_train + 1:
                return None
            signals = self._walk_forward(feat, fc, silent=True)
            if not signals:
                return None
            sdf = pd.DataFrame(signals)
            bt = Backtest(self.spread, self.swap, 0.0001, self.bpd)
    ...
    def _meta(self, core, feat):
        return {
            "tf": self.tf, "multiplier": self.multiplier, "target_bars": self.target_bars,
    ...
    def run(self, filepath=None, mode="signal"):
        ...
        {"signal": self.run_signal, "daily": self.run_daily,
         "dashboard": self._regen_dash, "research": self.run_research}.get(mode, self.run_research)(path)
...
def main():
    ...
    if args.fetch:
        ...
        log.info("Aucun fichier — téléchargement GLD (Gold ETF)")
